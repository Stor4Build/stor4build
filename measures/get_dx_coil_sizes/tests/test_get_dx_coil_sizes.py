...
from measure import GetDXCoilSizes
...
class TestGetDXCoilSizes:
    """Py.test module for GetDXCoilSizes."""
    ...
    def model_out_path(test_name) -> Path:
        return TestGetDXCoilSizes.run_dir(test_name) / "example_model.osm"

    @staticmethod
    def sql_path(test_name) -> Path:
        return TestGetDXCoilSizes.run_dir(test_name) / "run/eplusout.sql"

    @staticmethod
    def report_path(test_name) -> Path:
        return TestGetDXCoilSizes.run_dir(test_name) / "report.html"
    ...
    def setup_test(
        ...
        run_dir = TestGetDXCoilSizes.run_dir(test_name)
        run_dir.mkdir(parents=True, exist_ok=True)

        report_path = TestGetDXCoilSizes.report_path(test_name)
        if report_path.exists():
            report_path.unlink()

        assert model_in_path.exists()

        model_out_path = TestGetDXCoilSizes.model_out_path(test_name)
        ...
        if os.environ.get("OPENSTUDIO_TEST_NO_CACHE_SQLFILE"):
            sql_file = TestGetDXCoilSizes.sql_path(test_name)
            if sql_file.exists():
                sql_file.unlink()
    ...
    def test_with_drybulb_temp(self):
        ...
        TestGetDXCoilSizes.setup_test(test_name=test_name, idf_output_requests=idf_output_requests)

        model_out_path = TestGetDXCoilSizes.model_out_path(test_name)
        assert model_out_path.exists()
        assert epw_path.exists()
        assert TestGetDXCoilSizes.sql_path(test_name).exists()
        ...
        runner.setLastEnergyPlusSqlFilePath(openstudio.toPath(str(TestGetDXCoilSizes.sql_path(test_name))))
        ...
        start_dir = Path.cwd()
        # try:
        os.chdir(TestGetDXCoilSizes.run_dir(test_name))
