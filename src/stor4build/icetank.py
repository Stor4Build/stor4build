...
# Fluid properties
freezing_temp = {"water": 0.0, "simplewater": 0.0, "pcm2x2a": 2.0}
specific_heat = {"water": 4180.0, "simplewater": 4184.0, "pcm2x2a": 2000.0}
...
class IceTank(Simulation):
    ...
    def size(cls, name, baseline_results, **kwargs):
        ...
        energy_cols = [
            ...
            if "Chiller Evaporator Cooling Energy" in el
        ]
        flow_cols = [
            el
            for el in df.columns.values.tolist()
            if "Chiller Evaporator Mass Flow Rate" in el
        ]
        df = df.loc[(df["hour_of_day"] >= k0) & (df["hour_of_day"] < k1)]
        df["total_w"] = df[energy_cols].sum(axis=1)
        df["total_flow"] = df[flow_cols].sum(axis=1)
        dft = df.groupby("ordinal_day", as_index=False).agg(
            {"total_w": "sum", "total_flow": "mean"}
        )
        ...
        requested_num_tanks = (
            joules_to_kwh * requested_capacity / single_tank_capacity[storage_medium]
        )
        actual_num_tanks = int(math.ceil(requested_num_tanks))
        actual_capacity = (
            actual_num_tanks * single_tank_capacity[storage_medium] / joules_to_kwh
        )
